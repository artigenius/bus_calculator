PRODUCTS.certification = {
  ...
  effectCards:[
    ...
    {title:'Кадровые решения', text:'Модель оценивает снижение кадровых ошибок.'},
  ...
  periodNote:'В итог включён месячный эффект бенефитов и годовые эффекты найма.',
  modelNote:'Внутренние расходы включают ФОТ менеджера и специалиста по проекту, время сотрудников на прохождение опроса и налоги.',
  inputs:[
    ...
    {id:'benefitsSalary', label:'Средняя зарплата для бюджета бенефитов', unit:'₽/мес', def:170000, group:'advanced', step:1000,
      help:'Для бенефитов используется отдельная зарплата — 170 000 ₽, для найма — 150 000 ₽.'},
    ...
    return {
      ...
      effects:[
        {id:'e1', label:'Экономия на привлечении кандидатов', amount:e1,
          ...
          source:'Great Place to Work — рост откликов на 20%, консервативный коэффициент модели 6,7%; NACE Job Outlook — годовой найм 5%. Стоимость найма 18% годовой зарплаты — рыночное допущение.'
        },
        {id:'e2', label:'Эффект от снижения кадровых ошибок', amount:e2,
          logic:'Модель предполагает, что исследование целевой аудитории снижает вероятность кадровой ошибки на 10%.',
          data:[
          ...
          source:'По данным SHRM / BLS и CertifiedSource (2026), прямые издержки неудачного найма составляют 30–50% зарплаты за первый год. Для консервативной оценки используется нижняя граница — 30%, без косвенных потерь.'
        },
        {id:'e3', label:'Экономия от снижения текучести', amount:e3,
          ...
          source:'По статистике Great Place to Work уровень увольнений в сертифицированных компаниях ниже на 39%. В модели этот показатель разделён на 3 — до 13%.'
        },
        {id:'e4', label:'Эффективность бюджета на бенефиты', amount:e4,
          ...
          source:'Экспертная оценка Europlast в статье «Привычная награда», Коммерсантъ СПб, 17.12.2019 — бенефиты около 10% ФОТ. По данным GPTW, фокус на атрибутах, которые ценит целевая аудитория, повышает эффективность инвестиций в бренд на 10%. Для консервативной оценки показатель снижен до 3,3%.'
        }
      ],
      costBreakdown:[
